...
const sio2jailCommand = "timeout %.1fs %v -f 3 -o oiaug %v -- %v 3> %v"
...
func RunProcessWithOiejq(processID, command string, input io.Reader) (oiejqProcessInfo *ProcessInfo, err error) {
	...
	oiejqCommand := fmt.Sprintf(sio2jailCommand, timelimit.Seconds(), sio2jailPath, options, command, oiejqResults.Name())
